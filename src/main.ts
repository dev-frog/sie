...
import { readFileSync } from "fs";
import { TypeSIE, TypeSIE_KONTO, TypeSIE_KONTO_EXPORT } from "./typings/sie";
...
class sieParser {
  ...
  constructor(path: string) {
    ...
    this.sie = this.sieLines(buffer.toString());
    this.sieObject = this.parse();
  }

  private sieLines(sie: string): string[] {
    return sie.split("\n");
  }
  ...
  public getVerifications(): string[] {
    const elements = this.sieObject.VER;
